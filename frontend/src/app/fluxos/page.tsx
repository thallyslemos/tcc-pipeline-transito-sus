...
const FluxoMapView = dynamic(() => import("@/components/map/FluxoMapView"), {
  ssr: false,
});
...
function MunicipioCombobox({
  ...
  const [search, setSearch] = useState(
    value ? `${value.municipio} - ${value.uf}` : "",
  );
        ...
        <input
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "ArrowDown") {
              ...
              e.preventDefault();
              if (destacado >= 0 && destacado < results.length)
                selecionar(results[destacado]);
          ...
          aria-activedescendant={
            destacado >= 0 ? `fluxos-municipio-opt-${destacado}` : undefined
          }
        />
        <ChevronDown
          className="pointer-events-none absolute right-2.5 top-2.5 h-4 w-4"
          style={{ color: "var(--ink-2)" }}
        />
      </div>
          ...
          {results.map((m, i) => (
            <li
              ...
              style={{
                color: "var(--ink)",
                backgroundColor:
                  i === destacado ? "var(--brand-soft)" : "transparent",
              }}
...
export default function FluxosPage() {
  return (
    <Suspense
      fallback={
        <div
          className="flex h-96 items-center justify-center text-sm"
          style={{ color: "var(--ink-2)" }}
        >
          Carregando...
        </div>
...
function FluxosContent() {
  ...
  const [municipioSelecionado, setMunicipioSelecionado] =
    useState<SimMunicipio | null>(null);

  const [anos, setAnos] = useState<number[]>([]);
  const [tipos, setTipos] = useState<string[]>([]);

  const [fluxo, setFluxo] = useState<SimFluxo | null>(null);
  const [geoData, setGeoData] = useState<FluxoGeoFeatureCollection | null>(
    null,
  );
  ...
  useEffect(() => {
    fetchSimAnos("ocorrencia").then((r) => {
      ...
      if (recorte.ano == null && r.anos.length)
        patchRecorte({ ano: r.anos.at(-1) });
    });
  ...
  useEffect(() => {
    ...
    if (!codigo) return;
    fetchSimMunicipios(
      { dimensao: "ocorrencia", municipio: codigo },
      1,
      10,
    ).then((r) => {
      const encontrado = r.municipios.find(
        (m) => m.cod_mun_ibge.slice(0, 6) === codigo.slice(0, 6),
      );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err) =>
        setError(
          err instanceof Error ? err.message : "Erro ao carregar fluxos",
        ),
      )
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs" style={{ color: "var(--ink-2)" }}>
          Visualize a ligacao entre municipio de ocorrencia e municipio de
          residencia das vitimas
        </p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            <button
              ...
              style={{
                backgroundColor:
                  direcao === "origens" ? "var(--brand)" : "var(--surface)",
            ...
            <button
              ...
              style={{
                backgroundColor:
                  direcao === "destinos" ? "var(--brand)" : "var(--surface)",
                color:
                  direcao === "destinos" ? "var(--canvas)" : "var(--ink-2)",
              }}
            >
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            value={tipoVeiculo ?? ""}
            onChange={(e) =>
              patchRecorte({ tipo_veiculo: e.target.value || undefined })
            }
      ...
      <BarraDeRecorte
        chips={[
          ...(municipioSelecionado
            ? [
                {
                  rotulo: "Município alvo",
                  valor: `${municipioSelecionado.municipio} - ${municipioSelecionado.uf}`,
                },
              ]
            : []),
          {
        ...
        >
          {direcao === "origens" ? (
            <>
              ...
              <b>{municipioSelecionado.municipio}</b>. As linhas indicam os
              municipios de residencia das vitimas.
            </>
          ) : (
            <>
              <b>Destinos dos residentes:</b> municipio de residencia fixado em{" "}
              <b>{municipioSelecionado.municipio}</b>. As linhas indicam onde
              ocorreram os obitos dos seus residentes.
            </>
          )}
        </div>
      )}
      ...
      {error && !loading && (
        <div
          ...
          style={{
            backgroundColor: "var(--risk-1)",
            color: "var(--risk-5)",
            border: "1px solid var(--risk-5)",
          }}
        >
          {error}
        </div>
        ...
        >
          ...
          >
            ...
            >
              {direcao === "origens"
                ? "Municipios de residencia das vitimas"
                : "Municipios de ocorrencia dos residentes"}
            </h2>
            <p className="text-xs" style={{ color: "var(--ink-2)" }}>
              Top {fluxo.filtros.top_n} por obitos ·{" "}
              {fluxo.total_ambos_encontrados} obitos com ambas as geografias
              encontradas
            </p>
          </div>
              ...
              <tbody>
                {fluxo.arestas.map((edge, i) => (
                  <tr
                    ...
                    style={{
                      borderBottom: "1px solid var(--border)",
                      backgroundColor: edge.propria_municipio
                        ? "var(--brand-soft)"
                        : "transparent",
                    }}
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            edge.geografia_status === "encontrado"
                              ? "var(--ok-soft)"
                              : "var(--risk-1)",
                          color:
                            edge.geografia_status === "encontrado"
