...
export default function ForecastChart({ historico, previsao, height = 360 }: Props) {
  ...
  return <ResponsiveContainer width="100%" height={height}><ComposedChart data={data}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" /><XAxis dataKey="competencia" tick={{ fontSize: 10, fill: "var(--chart-axis)" }} /><YAxis tick={{ fontSize: 10, fill: "var(--chart-axis)" }} tickFormatter={(value) => String(Math.round(Number(value)))} /><ThemedTooltip formatter={(value: unknown, name: unknown) => value == null || name === "band" ? [null, null] : [Math.round(Number(value)).toLocaleString("pt-BR"), name === "real" ? "Dados observados" : "Projecao"]} /><Legend formatter={(value) => value === "real" ? "Dados observados" : value === "previsao" ? "Projecao" : "Intervalo"} /><Area dataKey="band" fill="var(--risk-2)" stroke="none" fillOpacity={0.6} name="band" /><Line dataKey="real" stroke="var(--risk-5)" strokeWidth={2} dot={false} name="real" /><Line dataKey="previsao" stroke="var(--attention)" strokeWidth={2} strokeDasharray="6 3" dot={false} name="previsao" /></ComposedChart></ResponsiveContainer>;
}
