...
def oklch_hex(L, C, H, clamp=True):
    ...
    if not ing and clamp:
        ...
        for _ in range(40):
            mid = (lo + hi) / 2
            aa = mid * math.cos(math.radians(H))
            bb2 = mid * math.sin(math.radians(H))
            rr, gg, bb3 = oklab_to_lrgb(L, aa, bb2)
            if all(-1e-4 <= v <= 1.0001 for v in (rr, gg, bb3)):
                lo = mid
            else:
                hi = mid
        C = lo
...
def dist(a, b):
    return (
        sum(
            (int(a[1 + 2 * i : 3 + 2 * i], 16) - int(b[1 + 2 * i : 3 + 2 * i], 16)) ** 2
            for i in range(3)
        )
        ** 0.5
    )
...
print(
    "   monotonica:",
    all(ys[i] > ys[i + 1] for i in range(4)),
    "| razao entre degraus:",
    " ".join(f"{ys[i] / ys[i + 1]:.2f}x" for i in range(4)),
)
...
for i, h in enumerate(RISK, 1):
    ...
    print(
        f"   risk-{i} {h}  tinta {a:5.2f}  canvas {b:5.2f}  -> usar {'tinta' if a >= b else 'canvas'}"
    )
