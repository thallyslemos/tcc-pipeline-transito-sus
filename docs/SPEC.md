...
| **SENATRAN** | Frota de veículos por município | SENATRAN | ⏳ Pendente | Óbitos por 10k veículos |
...
| Testes | pytest (54 testes passando) |
| Lint/Format | ruff |
...
### 3.3 Óbitos por 10 mil veículos (planejado)

```python
taxa_veiculos = (total_obitos / frota_municipio) * 10.000
```

- **Fonte frota**: SENATRAN (RENAVAM)
- **Status**: ⏳ Pendente — aguardando modelagem
...
|--------|--------|-----------|
| `run.py` | ✅ Implementado | Orquestrador CLI com flags `--sim-only`, `--ibge-only`, `--gold` |
...
| `/municipio` | ✅ | dimensao, regiao, uf, ano, municipio |
...
| Indicador: Óbitos/10k veículos | ⏳ | Aguarda SENATRAN |
...
| 2 | Integrar SENATRAN (óbitos/10k veículos) | 🟡 Média |
