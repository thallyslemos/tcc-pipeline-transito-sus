...
from .sample_data import MUNICIPIOS

logger = get_logger(__name__)


def _mun_case_expr(col: str) -> str:
    """Gera expressao CASE WHEN para nomes de municipio."""
    clauses = " ".join(f"WHEN {col} = '{c}' THEN '{m['nome']}'" for c, m in MUNICIPIOS.items())
    return f"CASE {clauses} ELSE 'Outro' END"
    ...
    con.sql(f"""
        COPY (
            SELECT
                ...
                {_mun_case_expr("cod_mun_ocorrencia")}          AS municipio,
                uf,
                competencia,
            ...
            GROUP BY cod_mun_ocorrencia, uf, competencia, tipo_veiculo, faixa_etaria, sexo_desc
    ...
    con.sql(f"""
        COPY (
            SELECT
                ...
                {_mun_case_expr("cod_mun")}                     AS municipio,
                uf,
                competencia,
                YEAR(competencia)                               AS ano,
                MONTH(competencia)                              AS mes,
            ...
            GROUP BY cod_mun, uf, competencia, tipo_veiculo, faixa_etaria
